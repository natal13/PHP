...
$sql = "SELECT user_id, last_name, first_name, email, registration_date FROM users";
$result = $dbc -> query($sql);
$number = mysqli_num_rows($result);
echo "<p style='font-size:14px'>Liczba zerejestrowanych użytkowników: " . $number . "</p> <br>";
echo "<table><th>Nazwisko</th><th>Imię</th><th>E-mail</th><th>Data rejestracji</th><th>Edycja</th><th>Usuwanie</th>";
if($result -> num_rows>0) {
    while($row = $result -> fetch_assoc()) {
        $id = $row["user_id"];
        echo "<tr><td>" . $row["last_name"] . "</td><td>" . $row["first_name"] . "</td><td>" . $row["email"] ."</td><td>" . $row["registration_date"] ."</td><td>" . "<li><a class='admin_users' href='edit.php?id=$id'>Edytuj</a></li>" ."</td><td>" . "<li><a class='admin_users' href='delete_submit.php?id=$id'>Usuń</a></li>" . "</td></tr>";
    }
}
else {
    echo "0 result";
